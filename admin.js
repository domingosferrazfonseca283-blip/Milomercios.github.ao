...
async function carregarVendedores() {
  ...
  $('total-ativos').textContent = vendedores.filter(v => v.subscricaoAtiva === true && v.estadoConta !== 'bloqueado').length;
  $('vendedores-list').innerHTML = vendedores.length ? vendedores.map(v => `
    <div class="admin-item">
      <div><strong>${escapeHtml(v.nomeLoja || 'Sem nome')}</strong><br><small>${escapeHtml(v.email || '')} · Conta: ${escapeHtml(v.estadoConta || 'pendente')} · Subscrição: ${v.subscricaoAtiva ? 'ativa' : 'inativa'}</small></div>
      <div><button class="btn" onclick="alternarVendedor('${v.id}',${v.subscricaoAtiva === true})">${v.subscricaoAtiva ? 'Suspender' : 'Ativar'}</button></div>
...
async function carregarProdutos() {
  ...
  $('produtos-list').innerHTML = produtos.length ? produtos.map(p => `
    <div class="admin-item"><div><strong>${escapeHtml(p.nome || 'Produto')}</strong><br><small>${Number(p.preco || 0).toLocaleString('pt-AO')} Kz · ${escapeHtml(p.categoria || 'outros')} · ${p.ativo === false ? 'inativo' : 'ativo'}</small></div>
    <button class="${p.ativo === false ? 'btn' : 'btn-danger'}" onclick="alternarProduto('${p.id}',${p.ativo !== false})">${p.ativo === false ? 'Publicar' : 'Ocultar'}</button></div>`).join('') : '<p>Nenhum produto publicado.</p>';
}

window.alternarVendedor = async (id, ativo) => {
  await updateDoc(doc(db, 'usuarios', id), { subscricaoAtiva: !ativo, estadoConta: !ativo ? 'ativo' : 'bloqueado', atualizadoEm: serverTimestamp() });
  await carregarVendedores();
  alert(!ativo ? 'Vendedor ativado.' : 'Vendedor suspenso.');
};
window.alternarProduto = async (id, ativo) => {
  await updateDoc(doc(db, 'produtos', id), { ativo: !ativo, atualizadoEm: serverTimestamp() });
  await carregarProdutos();
};
